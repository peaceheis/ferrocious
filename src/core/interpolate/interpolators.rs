...
mod tests {
    ...
    fn test_constant_interpolator() {
        ...
        let result = interp.evaluate(&TimeStamp::new(0, 5, 0), 24);
    ...
    fn test_linear_interpolator() {
        ...
        // At start
        assert_eq!(interp.evaluate(&TimeStamp::new(0, 0, 0), 24), 0.0);
        // At end
        assert_eq!(interp.evaluate(&TimeStamp::new(0, 1, 0), 24), 10.0);
        // At midpoint (0.5 seconds = 12 frames at 24fps)
        let mid = interp.evaluate(&TimeStamp::new(0, 0, 12), 24);
    ...
    fn test_cubic_interpolator() {
        ...
        // At start
        let start = interp.evaluate(&TimeStamp::new(0, 0, 0), 24);
        assert!((start - 0.0).abs() < 0.01);

        // At end
        let end = interp.evaluate(&TimeStamp::new(0, 1, 0), 24);
    ...
    fn test_cubic_works_with_arrays() {
        ...
        let start_color = color_curve.evaluate(&TimeStamp::new(0, 0, 0), 24);
        assert_eq!(start_color, [1.0, 0.0, 0.0, 1.0]);

        let end_color = color_curve.evaluate(&TimeStamp::new(0, 1, 0), 24);
        assert_eq!(end_color, [0.0, 0.0, 1.0, 1.0]);

        // Middle should be some blend influenced by the orange/purple controls
        let mid_color = color_curve.evaluate(&TimeStamp::new(0, 0, 12), 24);
    ...
    fn test_custom_interpolator_via_trait() {
        ...
        // At t=0, sin(0) = 0
        assert!((wave.evaluate(&TimeStamp::new(0, 0, 0), 24) - 0.0).abs() < 0.01);

        // At t=0.25s, sin(π/2) ≈ 1
        let quarter = wave.evaluate(&TimeStamp::new(0, 0, 6), 24); // 6 frames at 24fps = 0.25s
    ...
    fn test_both_approaches_are_compatible() {
        ...
        let builtin_result = builtin.evaluate(&TimeStamp::new(0, 0, 12), 24);
        let custom_result = custom.evaluate(&TimeStamp::new(0, 0, 12), 24);
